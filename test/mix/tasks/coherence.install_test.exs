...
defmodule Mix.Tasks.Coherence.InstallTest do
  ...
  test "generates files for authenticatable recoverable unlockable_with_token" do
    in_tmp(
      "generates_files_for_authenticatable_recoverable_registerable_unlockable_with_token",
      fn ->
        mk_web_path()
  ...
  describe "installed options" do
    ...
    test "install options many" do
      in_tmp("install options many", fn ->
        ...
        assert output ==
                 "mix coherence.install --confirmable --rememberable --registerable --invitable --authenticatable --recoverable --lockable --trackable --unlockable-with-token"
      end)
    end
  end
